console.log('') 
console.log('|ON| | Bot activated ')
console.log('-------------------------------') 
console.log(' ● Скрипт запущен.') 
console.log('  * Started - Работает')
console.log('  * Error# - Не работает')
console.log('-------------------------------') 
console.log('[ϟ] Info-Board: Bot online | Ошибок не обнаружено')
console.log('')
...
let user = new VK();
user.setOptions({
	token: '203f6fc6ee0a9d8bd7f904400b7bcc524a69da589b0bfa26a60b3d4794bd1742be2d59caf966b7a765c12' // token ot страницы ( не обязательно )
});
let giving = false;
const cars = [
	{
...
vk.setOptions({ token: '203f6fc6ee0a9d8bd7f904400b7bcc524a69da589b0bfa26a60b3d4794bd1742be2d59caf966b7a765c12', pollingGroupId:202061318 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club202061318\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club202061318\|(.*)\]/ig, '').trim();

	if(!users.find(x=> x.id === message.senderId))
	{
		message.send (`[👋🏻]Привет,я игровой бот "Подручный".
...
cmd.on(/^(?:оборудование)$/i, async (message, bot) => {
	...
	let id = message.user.id
       ...
       ⛔ Страйки: ${message.user.straik}
		...
		Кнопки:\n`+
		(message.user.serkn== true ? ` ` : `Серебрянная кнопка: Получена\n`)+
...
cmd.on(/^(?:микрофон)$/i, async (message, bot) => {
	let user = message.user 
     let id = message.user.id
		...
		if(message.user.kanal == true){
 		if(message.user.ymoney < 500) return message.send(`${message.user.tag}, У вас не достаточно денег цена микрофона 1₽ .`);
...
cmd.on(/^(?:бонус|@club202061318|💎Бонус )$/i, async (message, bot) => {
...
➾➖➾➖➾➖➾➖➾➖➾
[😉] » Чаты » бот даст список беседе
[🔥] » Сервер » Бот поделится с нашим сервером
[😎] » Правила » Бот поделится с вами правилами 
➾➖➾➖➾➖➾➖➾➖➾
   [🎉] » Развлекательные:
[↪] » Переверни [фраза]
[🔮] » Шар [фраза]
...
🔹Что бы купить донат, перейдите по ссылке https://vk.com/topic-202061318_46893857
`)
});
...
🔹Что бы купить Валюту, перейдите по ссылке https://vk.com/topic-202061318_46893857
`)
});
/////
...
return message.send(`Держи годный трек , Йоу`, { attachment: utils.pick(["audio495753877_456243594", "audio495753877_456243593", "audio495753877_456243600", "audio495753877_456243583", "audio495753877_456243526", "audio495753877_456243393", "audio2001011109_80011109"]) });  
});

cmd.on(/^(?:рандом пикча|пикча)$/i, async (message) => { 
return message.send(`Держи годную пикчу , бро`, { attachment: utils.pick(["photo-75214966_457584486", "photo-75214966_457584484", "photo-75214966_457584478", "photo-75214966_457584472", "photo-81250721_456240104", "photo-45045130_457995542", "photo-61166138_457279981"]) }); 
});
...
cmd.on(/^(?:сервер)$/i, async (message, bot) => {
	message.user.foolder += 1;
 return bot(`
🔻 ➾ Наш сервер в сампе 0.3.7« 🔻 
🙂ip: 83.222.105.60:8807🙂
🔥Зови своих друзей, админы добрые и хорошие🔥
   `)	
});
...
cmd.on(/^(?:Чаты|Беседы)/i, async (message, args, bot) => {
	message.user.foolder += 1;
message.send(`
1.Беседа https://vk.me/join/E_tXnloZfbiBHNEDhOClggWa_6PlLJl184c=
2.Беседа
 ☺😉напишите https://vk.com/kubasov2020 ему что вы хотите свою беседу добавить
...
cmd.on(/^(?:Статистика)$/i, async (message, bot) =>{
	message.user.foolder += 1;
    return bot(`
	 🔧 Техническая информация бота https://vk.com/pipupap (Подручный) 🔧
⚙ » OC: Windows 10
🔐 » Версия бота: 2.5
...
cmd.on(/^(?:фермы|@club202061318🔋 Ферма )\s?([0-9]+)?$/i, async (message, bot) => {
	message.user.foolder += 1;
